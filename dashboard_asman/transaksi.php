...
<body class="sb-nav-fixed">
                        ...
                        <div class="card-body">
                            ...
                            <div style="overflow-x: auto;">
                                <table id="datatablesSimple" class="table table-bordered table-striped table-hover">
                                    ...
                                    $tampil = mysqli_query($koneksi, "SELECT p.*, u.* FROM pengajuan p  JOIN user u on u.nik= p.id_author  WHERE p.status='Peminjaman Selesai' ORDER BY id_pengajuan DESC");
                                    ...
                                    ?>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php if ($data['status'] == 'Peminjaman Selesai') {
                                                    echo $data['status'] ?>
                                                    <a href="#" style="text-decoration: none;" class="badge text-bg-primary" data-bs-toggle="modal" data-bs-target="#modalDetailProses<?= $no ?>">Detail Proses</a>
                                                <?php } else {
                                                    echo $data['status'];
                                                } ?>
                                        ...
                                        ?>
                                        <!-- Awal Modal Detail Proses -->
                                        <div class="modal fade modal-lg justify-content-center" id="modalDetailProses<?= $no ?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                            <div class="modal-dialog modal-xl">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="staticBackdropLabel">Alur Proses Form PPKO</h5>
                                        ...
                                        <!-- AKhir Modal Detail Proses-->
                                    <?php endwhile ?>
                                </table>
                                <div class="modal-footer">
                                    <!-- <button type="button" class="btn btn-primary" onclick="printModalContent()">Cetak</button> -->
                                    <a href="../cetak1.php?id=<?php echo $data['id_pengajuan']; ?>" target="_blank">Cetak PDF</a>
                                </div>
                                <div class="modal-footer">
                                    <!-- <button type="button" class="btn btn-primary" onclick="printModalContent()">Cetak</button> -->
    ...
    <!-- Script Total Biaya -->
    <script>
        const biayaBensinInput = document.getElementById("biayaBensin");
        const biayaTolInput = document.getElementById("biayaTol");
        ...
        biayaBensinInput.addEventListener("input", updateTotalBiaya);
        biayaTolInput.addEventListener("input", updateTotalBiaya);
        biayaLainInput.addEventListener("input", updateTotalBiaya);
        uangJalanInput.addEventListener("input", updateTotalBiaya);


        function updateTotalBiaya() {
            const biayaBensin = parseFloat(biayaBensinInput.value) || 0;
            const biayaTol = parseFloat(biayaTolInput.value) || 0;
            const biayaLain = parseFloat(biayaLainInput.value) || 0;
            const uangJalan = parseFloat(uangJalanInput.value) || 0;

            const totalBiaya = uangJalan - biayaBensin - biayaTol - biayaLain;

            totalBiayaSpan.textContent = " " + totalBiaya.toLocaleString('id-ID', {
                style: 'currency',
                currency: 'IDR'
            });
        }
